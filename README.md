...
```sh
npm install git+ssh://git@github.com:stainless-sdks/miriam-example-typescript.git
```
...
const client = new MiriamExample({
  apiKey: process.env['MIRIAM_EXAMPLE_API_KEY'], // This is the default and can be omitted
});
...
1. Via the `MIRIAM_EXAMPLE_LOG` environment variable
...
const client = new MiriamExample({
  logger: logger.child({ name: 'MiriamExample' }),
...
const client = new MiriamExample({ fetch });
```
...
```ts
import MiriamExample from 'npm:miriam-example';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/miriam-example-typescript/issues) with questions, bugs, or suggestions.
